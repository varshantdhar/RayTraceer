...
stage get_stage(unsigned int selection)
{
  ...
  if (selection == 50) {
	  ...
	  object *pst = sphere_text(xyz_expr(0,0,3),1,&map_texture_on_sphere,func,color_expr(0,0,0));
  ...
  if (selection == 51) {
	  ...
	  object *pst = sphere_text(xyz_expr(0.5,0.7,3),0.5,&map_texture_on_sphere,func,color_expr(0,0,0));
	  scene s = {{0, 0, 0.25}, ol_singleton(*pst), l, amb};
	  free(pst);
	  stage g = {c, s};
	  return g;
